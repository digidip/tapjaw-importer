...
class TapjawHttpConnector {
    ...
    async post(uri, query, body, headers) {
        ...
        const options = Object.assign({ hostname: this.host, port: this.port, path: `${uri}?${querystring.stringify(query)}`, method: 'POST' }, Object.assign({}, headers, { 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(body) }));
    ...
    async postJson(uri, query, json, headers) {
        ...
        const options = Object.assign({ hostname: this.host, port: this.port, path: `${uri}?${querystring.stringify(query)}`, method: 'POST' }, Object.assign({}, headers, { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(json) }));
        return this.getResponse(options);
    }
    /**
