...
class TapjawHttpConnector {
    ...
    setDecoding(encoding) {
        ...
        if (!(0, iconv_lite_1.encodingExists)(encoding)) {
            throw new tapjaw_connector_error_1.TapjawConnectorError(`Unsupported decoding: ${encoding || 'not set'}`, this);
        }
    ...
    setEncoding(encoding) {
        ...
        if (!(0, iconv_lite_1.encodingExists)(encoding)) {
            throw new tapjaw_connector_error_1.TapjawConnectorError(`Unsupported encoding: ${encoding || 'not set'}`, this);
        }
    ...
    getProtocolRequest() {
        switch (true) {
            ...
            default:
                throw new tapjaw_connector_error_1.TapjawConnectorError(`Invalid protocol: ${String(this.protocol)}`, this);
        }
    }
    /**
        ...
        return new Promise((resolve, reject) => {
            const connectorRequest = this.getProtocolRequest()(options, (response) => {
                ...
                if (response.statusCode !== 200) {
                    const error = new tapjaw_connector_error_1.TapjawConnectorError(`HTTP Status code was ${response?.statusCode || 'not set'}.`, this);
                    reject(error);
                }
                const buffer = [];
                ...
                response.on('end', () => {
                    ...
                    if (!contentBuffer) {
                        reject(new tapjaw_connector_error_1.TapjawConnectorError('Empty content buffer', this));
                    }
                    if (this.enableGzip) {
            ...
            connectorRequest
                ...
                reject(new tapjaw_connector_error_1.TapjawConnectorError(`${options.hostname || 'not set'} Timed out after ${options.timeout || '(not set)'}ms`, this));
            })
                .on('error', reject);
            if (writeBody) {
