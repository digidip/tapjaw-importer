...
class ApplyAuthorizationHttpHeaderWrapper {
    ...
    authenticate(requestOptionContainer) {
        return new Promise(async (resolve, reject) => {
            ...
            this.authenticationData = await this.authenticator.authenticate().catch(reject);
            if (!this.authenticationData) {
                return reject(new tapjaw_authenticator_1.TapjawAuthenticatorError(`No Authorization header data recieved from authenticator: ${this.authenticator.constructor.name || 'Unknown'}`));
            }
