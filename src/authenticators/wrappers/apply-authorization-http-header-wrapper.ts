...
export default class ApplyAuthorizationHttpHeaderWrapper implements TapjawAuthenticationWrapper {
    ...
    public authenticate(requestOptionContainer: https.RequestOptions): Promise<https.RequestOptions> {
        return new Promise(async (resolve, reject) => {
            ...
            this.authenticationData = await this.authenticator.authenticate().catch(reject) as HttpHeaders;
            if (!this.authenticationData) {
                return reject(new TapjawAuthenticatorError(`No Authorization header data recieved from authenticator: ${this.authenticator.constructor.name || 'Unknown'}`));
            }
