...
class MockResponse extends EventEmitter implements Pick<IncomingMessage, 'statusCode' | 'setEncoding'> {
    write = jest.fn();
    end = jest.fn();
    statusCode = 200;
...
describe('Make sure requests() works as expected', () => {
    test('should be able to handle simple JSON string', async () => {
        const spy = jest.spyOn(https, 'request').mockImplementation(
            // @ts-ignore
            (options: RequestOptions, callback?: (res: IncomingMessage) => void): any => {
